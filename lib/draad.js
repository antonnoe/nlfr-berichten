...
function leesBlok(id, binnen, postUrl) {
  ...
  const profiel = (dt.match(/href=["']([^"']*\/profile\/[^"']+)["']/i) || [])[1] || "";
  const avatar = (dt.match(/<img[^>]+src=["']([^"']+)["']/i) || [])[1] || "";
...
// Ning pagineert de reacties met ?page=N#comments op de blog-slug.
function vervolgPaginas(html) {
  const uit = new Set();
  const re = /href=["']([^"']*[?&]page=(\d+)[^"']*)["']/gi;
  let m;
  while ((m = re.exec(html))) {
    const href = ontsnap(m[1]);
    if (!/#comments|\/profiles\/blogs\//i.test(href)) continue;
    const n = parseInt(m[2], 10);
    if (n > 1 && n <= MAX_PAGINAS) uit.add(n);
  }
...
export async function haalDraad(postId, opties) {
  ...
  // Vervolgpagina's ophalen tot we alles hebben (of de bovengrens raken).
  const paginas = slug ? vervolgPaginas(html) : [];
  const opgehaald = [];
  for (const n of paginas) {
    if (verwacht && gevonden.size >= verwacht) break;
    try {
      const extra = await haalPagina(slug + "?page=" + n);
      lees(extra);
      opgehaald.push(n);
    } catch (e) {
      break;
  ...
  if (debug) {
    uit.debug = { lengte: html.length, slug, paginas, opgehaald, verwacht };
  }
  return uit;
}
